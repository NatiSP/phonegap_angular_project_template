...
mod.controller('NavigationController', function($scope, $location) {
	$scope.onClickScan = function() {
		if(! confirm('Möchten Sie die Seite verlassen?')) return;
		$location.path('/scan');	
	}
...
mod.controller('InitializeController', function($scope, $http, $location, appModel, ws) {
	...
	this.loadAppData = function() {
		...
		var dataPath = "http://socian.net/" + appModel.data.path + "/data.json?r=" + Math.random(1);
	...
	this.checkInternetConnection = function() {
		$scope.status = "Prüfe WI-FI ...";
		if(appModel.data.config['wificheck'] == "true") {
			// check the internet connection	
			var networkState = navigator.network.connection.type;
			if(networkState != Connection.WIFI) {
				alert('Sie müssen mit dem lokalen WI-FI verbunden sein');
				$location.path('/start');
			}
	...
	this.createWSConnection = function() {
		$scope.status = "Prüfe Web Socket ...";
		ws.onopen = function() {
			$scope.$apply(function() {
				if(orderMode == "single") $location.path('/menuorder');
				else if(orderMode == "multi") $location.path('/multiorder');	
			})
		}
	...
	this.handler[ OrderCommand.ORDER_UPDATE ] = function(data) {
		if(data == null) {
			...
			$scope.$apply(function() {
				...
				$scope.items = orderModel.data.items;
				$scope.orderTotal = orderModel.data.total;
				$scope.orderStatus = parseInt(orderModel.data.status);	
				$scope.orderId = orderModel.data.orderid;
				
				if($scope.orderStatus == 2) {
					alert('Iher Bestellung kann abgeholt werden');
				}
...
//-------------------------------------------------------------//
//      Controller for handling multiple orders
//-------------------------------------------------------------//

mod.controller('MultiOrderController', function($scope, $http, $location, appModel, orderModel, ws) {
